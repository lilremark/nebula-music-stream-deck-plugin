...
export function steppedPlaybackRate(current: number, ticks: number): number {
  return clamp(Math.round((current + ticks * 0.1) * 10) / 10, 0.5, 2);
}

export function steppedPitch(current: number, ticks: number): number {
  return clamp(Math.round(current + ticks), -12, 12);
}
